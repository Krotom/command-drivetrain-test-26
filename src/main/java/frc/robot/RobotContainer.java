...
public class RobotContainer {
    ...
    public RobotContainer() {
      ...
      // TODO we dont use swerve right now, implement swerve later
      // sike! im using tank drive with pathplanner now :)
      ...
      try {
        ...
        e.printStackTrace();
        throw new RuntimeException("Failed to load path file(s) for autonomous");
      }

      autonomousCommand = new GoToTargetCommand(m_driveSubsystem, path.getStartingDifferentialPose(), true).andThen(AutoBuilder.buildAuto("MainAuto"));
